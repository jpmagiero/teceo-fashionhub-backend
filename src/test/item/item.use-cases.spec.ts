...
describe('Item Use Cases', () => {
  describe('CreateItemUseCase', () => {
    ...
    it('should create an item successfully', async () => {
      ...
      expect(result.status).toBe(ItemStatus.EM_ESTOQUE);
  ...
  describe('BulkUpdateStatusUseCase', () => {
    ...
    it('should update status in bulk', async () => {
      const mockItems: Item[] = [
        { id: 1, status: 'em_estoque' } as Item,
        { id: 2, status: 'em_estoque' } as Item,
      ];
      repo.bulkUpdateStatus.mockResolvedValue(mockItems);

      const result = await useCase.execute([1, 2], 'em_estoque');

      expect(result.length).toBe(2);
      expect(result[0].status).toBe('em_estoque');
    });
  });
});
